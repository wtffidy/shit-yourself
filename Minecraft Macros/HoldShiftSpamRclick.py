...
def spam_right_click_while_holding_shift():
    shift_held = False
    ...
    while True:
        if spamming.is_set():
            ...
            time.sleep(0.005)  # ~200 clicks/sec — adjust as needed
        else:
            if shift_held:
...
# --- GUI Setup ---
root = tk.Tk()
root.title("Hold Shift + Spam Right Click")
...
# --- Threads ---
threading.Thread(target=spam_right_click_while_holding_shift, daemon=True).start()
